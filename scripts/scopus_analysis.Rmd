...
# Figure 3 for SeaLifeBase
```{r}

# Make bubble plot worldview of locations of sealifebase citations
bubble_map <- leaflet(data = country) %>%
  setView(lng = 0, lat = 0, zoom = 2) %>%
  addProviderTiles("CartoDB.Positron") %>%  # Choose a tileset
  addCircleMarkers(
    radius = ~sqrt(number_documents) * 2,  # Adjust the scaling factor as needed
    color = "blue",
    fillOpacity = 0.7,
    popup = ~paste("Country/Territory: ", `COUNTRY/TERRITORY`, "<br>Number of Documents: ", number_documents)
  )
